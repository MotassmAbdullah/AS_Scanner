...
if __name__ == '__main__':
    ...
    _, _, _, _, L10 = st.columns(5)
    Ar = st.toggle('عربية :flag-sa:', )

    if Ar:
        st.sidebar.progress(3, "عداد المشتركين ( 2,000,000 $$)")
        st.title("المصحح الآلي")
        st.sidebar.info('1/1/2024 نهاية الاشتراك')
        st.sidebar.info("0.1.0 (تطبيق خاص)")
        st.sidebar.success("+966503029722 للتواصل")

        if st.session_state.page == 0:
            st.subheader("أهلا بك")
            st.divider()
            ...
            if st.session_state.input_im == "1 نموذج" and st.session_state.sup is None:
            ...
            elif st.session_state.input_im == "نموذج 2" and st.session_state.sup is None:
                ...
                st.session_state.Field = ["National ID", "School ID", "Administration ID", "Course", "Student Grade",
                                          ...
                                          "Answers 3", "Answers 4"]
            elif st.session_state.input_im == "ارفع الملف" and st.session_state.sup is None:
                file_up = st.file_uploader("إرفع الصورة هنا", accept_multiple_files=False)
                if file_up is not None:
                    st.session_state.img = cv2.imdecode(np.fromstring(file_up.getvalue(), np.uint8),
                                                        cv2.IMREAD_GRAYSCALE)
                else:
                    st.error("تأكد من الملف المرفوع")
                    with st.spinner():
                        st.stop()
                st.info("تأكد من تحديد مناطق الاحل")
                st.session_state.Orin = ["Vertical", "Vertical", "Vertical", "Vertical", "Vertical", "Vertical",
                                         "Vertical", "Vertical", "Vertical", "Horizontal", "Horizontal", "Horizontal",
                                         "Horizontal"]
                st.session_state.Field = ["National ID", "School ID", "Administration ID", "Course", "Student Grade",
            ...
            with L1:
                ...
                if start:
                    ...
                    prog = st.progress(0, text='إنتظر رجاءً')
                    T = [[]]
                    for r in range(len(st.session_state.data)):  # [0]:#
                        ...
                        if r < 3:
                            ...
                            re = ASS(st.session_state.img, st.session_state.data[r], st.session_state.Orin[r])
                            if re == '0':
                                T[0].append("سادس إبتدائي")
                            elif re == '1':
                                T[0].append("ثالث متوسط")
                        elif r == 5:
                            re = ASS(st.session_state.img, st.session_state.data[r], st.session_state.Orin[r])
                            if re == '0':
                                T[0].append("أ")
        ...
        if st.session_state.page == 13:
            ...
            with L2:
                if st.button("Submit", type="primary", use_container_width=True):
                    if st.session_state.input_im == "1 نموذج":
        ...
        st.sidebar.progress(3, "Memberships ( 2,000,000 $$)")
        st.title("AS Scanner")
        st.sidebar.info('License expiry date 1/1/2024 ')
        st.sidebar.info("0.1.0 (Privet app)")
        st.sidebar.success("Contact +966503029722")
        ...
        if st.session_state.page == 0:
            ...
            if st.session_state.sup is None:
                ...
                st.session_state.input_im = st.radio("*Input*",
                                                     ...
                                                     captions=["EM Form",
                                                               "EM Solved Form",
                                                               ""])
            st.text("<<<Preview image in the side bar<<<")
            st.sidebar.title("Preview إستعراض")
            if st.session_state.input_im == "Example 1" and st.session_state.sup is None:
                st.session_state.img = cv2.imread("./img_1.jfif", cv2.IMREAD_GRAYSCALE)
                st.session_state.data = [[152, (192 + 152), 368, (190 + 368), 10, 10],
                                         [152, (192 + 152), 260, (99 + 260), 5, 10],
                                         [152, (192 + 152), 209, (40 + 209), 2, 10],
                ...
                st.session_state.Field = ["National ID", "School ID", "Administration ID", "Course", "Student Grade",
                                          "Form", "School type",
                                          "Sex", "Nationality", "Answers 1", "Answers 2", "Answers 3", "Answers 4"]
            elif st.session_state.input_im == "Example 2" and st.session_state.sup is None:
                st.session_state.img = cv2.imread("./img_2.jpg", cv2.IMREAD_GRAYSCALE)
                st.session_state.data = [[152, (192 + 152), 368, (190 + 368), 10, 10],
                                         [152, (192 + 152), 260, (99 + 260), 5, 10],
                                         [152, (192 + 152), 209, (40 + 209), 2, 10],
                ...
                st.session_state.Field = ["National ID", "School ID", "Administration ID", "Course", "Student Grade",
                                          "Form", "School type", "Sex", "Nationality", "Answers 1", "Answers 2",
                                          "Answers 3", "Answers 4"]
            elif st.session_state.input_im == "Upload Images" and st.session_state.sup is None:
                file_up = st.file_uploader("Upload your image", accept_multiple_files=False)
                if file_up is not None:
                    st.session_state.img = cv2.imdecode(np.fromstring(file_up.getvalue(), np.uint8),
                                                        cv2.IMREAD_GRAYSCALE)
                else:
            ...
            with L1:
                ...
                if start:
                    # img_crop = st.session_state.img[10:d[2], d[3]:d[4]]
                    prog = st.progress(0, text='Please wait!!ً')
                    T = [[]]
                    for r in range(len(st.session_state.data)):  # [0]:#
                        prog.progress(int(200 / (len(st.session_state.data) - r + 1)))
                        if r < 3:
        ...
        if st.session_state.page == 13:
            ...
            with L2:
                if st.button("Submit", type="primary", use_container_width=True):
                    if st.session_state.input_im == "Example 1":
                        st.session_state.sup = 0
                    elif st.session_state.input_im == "Solved Ex نموذج محلول":
                        st.session_state.sup = 1
                    elif st.session_state.input_im == "Upload ارفع الملف":
